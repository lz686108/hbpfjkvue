<template>
  <div style="font-size: 0.5vw">
    <div class="crigca">
      ...
      align-items: center;width: 10vw;margin-right: 0.4vw;
">
        <div>
...
export default {
  ...
  methods: {
    ...
    echarts_list(atd1, atd2, atd3, atd4) {
      ...
      this.times = setInterval(async () => {
        ...
        atd1.shift()
        atd1.push(tnuir)
        this.time1eqs = (tnuir*1).toFixed(1);
        //10分钟
        let time = new Date();
        ...
        if (a1 != c1) {
          ...
          if (atc3[0].timeID.substring(10, 16) == atd2[atd2.length-1]) {
            ...
            his.time2eqs = atc3[0].v.toFixed(1)
          } else {
            //找到最后一次整点的位置
            let b = 0;
            ...
            this.time2eqs = (b1 / (atd2.length - b - 1)).toFixed(1);
          }
        }
        myChart.setOption({
          xAxis: {
  ...
  async mounted() {
    ...
    console.log(atd1,"atd1")
    console.log(atd2,"atd2")
    //小时timeid和v数组
    //小时timeid
    let atd4 = [];
    let atc3 = await this.new_data1(this.xid);
